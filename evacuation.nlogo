...
to initialisation-patches
  import-pcolors "images/stade_simple.png"
...
to move
  ...
  if pcolor = green_ [ set escaped? true ]


  let insight_green patches in-radius 10 with [pcolor = green_]
  if any? insight_green
    [ set heading towards one-of insight_green ]

  let insight_red patches in-radius 10 with [pcolor = red]
  if any? insight_red
    [ set heading towards one-of insight_red
      rt 180
  ]

  let near (other individus) in-radius 10 with [alive?]
